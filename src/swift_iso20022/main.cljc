(ns swift_iso20022.main
  "Kotodama WASM entrypoint for the Swift_iso20022 clean-room actor (L4) — Clojure port.
  ...
  facts (`swift_iso20022.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "swift_iso20022")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"swift_iso20022.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "swift_iso20022-compat" :tier tier :entities entities} 200])
...
(defn start! [] :swift_iso20022-compat/ready)
